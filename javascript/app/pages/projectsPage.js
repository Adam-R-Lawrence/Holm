...
function renderProjectsEmptyState(container, message) {
    container.innerHTML = '';
...
function createProjectDirectoryCard(project) {
    ...
    const descriptionText = getLocalizedText(project.description, project.description?.english || '');
...
function renderProjectDirectoryPage(grid, projects) {
    grid.innerHTML = '';

    (projects || []).forEach(project => {
        if (project) {
            grid.appendChild(createProjectDirectoryCard(project));
        }
    });
...
export async function loadProjectsPage() {
    ...
    try {
        ...
        renderProjectDirectoryPage(grid, projects);
    } catch (error) {
